...
class PieceFunction:
    ...
    '''
    def __init__(self, start, end, equal, function) -> None:
    ...
    def __input_verification(self,start, end, equal, function):
        if not isinstance(start, (int,float)):
            raise WrongType("start", type(start), "int/float")
        if not isinstance(end, (int,float)):
            raise WrongType("end", type(end), "int/float")
        if not start<end:
            raise RangeError(start, end)
        ...
        if not isinstance(function, str):
            raise WrongType("function", type(end), "str")
        if not math_function_regex.match(function):
            raise FunctionFormat(function)

    def eval(self, x, plot_mode=False):
        if plot_mode:
...
class PiecewiseFunction:
    ...
    def __input_verification(self,function_list):
        if not isinstance(function_list, list):
            raise WrongType("function_list", type(function_list), "list")
    ...
    def remove_fun(self, index):
        fl_len=len(self.__function_list)
        if index>=fl_len:
            raise ListIndexOutOFRange(index,fl_len)
        del self.__function_list[index]

    def get_fun(self, index):
        fl_len=len(self.__function_list)
        if index>=fl_len:
            raise ListIndexOutOFRange(index,fl_len)
        return self.__function_list[index]

    def plot_fun(self, index):
        fl_len=len(self.__function_list)
        if index>=fl_len:
            raise ListIndexOutOFRange(index,fl_len)
    ...
    def get_min(self):
        temp_min=self.__function_list[0].get_min()
        temp_arg_min=0
        for i in range(1,len(self.__function_list)):
            if self.__function_list[i].get_min() < temp_min:
                temp_min=self.__function_list[i].get_min()
                temp_arg_min=i
        return (temp_min, temp_arg_min)
    
    def get_max(self):
        temp_max=self.__function_list[0].get_max()
        temp_arg_max=0
        for i in range(1,len(self.__function_list)):
            if self.__function_list[i].get_max() > temp_max:
                temp_max=self.__function_list[i].get_max()
                temp_arg_max=i
        return (temp_max, temp_arg_max)
    ...
    def plot(self):
        ...
        for f in self.__function_list:
            f.plot(show=False,f_color=next(color_pool))
        plt.show()
